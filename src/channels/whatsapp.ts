...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    async function downloadInboundMedia(
      ...
      attachments: Array<{ type: string; name: string; mediaType: string; data: string }>;
      failures: string[];
    }> {
      ...
      const results: Array<{ type: string; name: string; mediaType: string; data: string }> = [];
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        try {
          ...
          // this guard, a `..`-laden fileName escapes the inbox on path.join.
          const rawFilename = normalized[key].fileName;
          let fallback = `${type}-${Date.now()}${ext}`;
          let mediaType = normalized[key].mimetype || 'application/octet-stream';

          // Images: normalize + shrink to keep the base64 payload (and the
          // Claude vision request) small, then hand over as image/jpeg so the
          // agent-runner sends it as a multimodal block (fork: image-vision).
          if (type === 'image') {
            ...
            fallback = `image-${Date.now()}.jpg`;
          }

          const safeRaw: string | undefined = type === 'image' ? undefined : rawFilename;
          const filename = typeof safeRaw === 'string' && isSafeAttachmentName(safeRaw) ? safeRaw : fallback;
          if (safeRaw && filename !== safeRaw) {
          ...
          // Hand the bytes to the host as base64; session-manager writes them
          // into the container-mounted session inbox and rewrites to localPath.
          results.push({ type, name: filename, mediaType, data: (buffer as Buffer).toString('base64') });
          log.info('Media downloaded', { type, filename, bytes: (buffer as Buffer).length });
        } catch (err) {
